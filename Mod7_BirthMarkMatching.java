...
public class Mod7_BirthMarkMatching {
...
//read large mered graph
 // read the graph from the file
         RegularFile fm = new RegularFile("MergedGraph"+"/"+"Merged_Graph.dhdf");

          Gmerged = new GrphBinaryReader().readGraph(fm);
           Gmerged.display();
            
         File file = new File("FSM_SUBGRAPH_SELECTED");
         String filepath="FSM_SUBGRAPH_SELECTED";
         String list[];
         String sCurrentLine;     
         BufferedReader br = null;
         ...
         if(file.isDirectory())
         {
           ...
           if(new File(filepath+"/"+list[i]).isFile() && list[i].endsWith("dhdf"))
           {  System.out.println("\n\nCurrent file="+list[i]); 
              RegularFile f = new RegularFile(filepath+"/"+list[i]); 
              total_no_birthmarks++;
              G=new GrphBinaryReader().readGraph(fm);
               //if(isIsomorphicTo(G,Gmerged))
               if(Gmerged.contains(G))
               //Collection<VertexPair> edgePairs = G.getEdgePairs();
               //if(G.isIsomorphicTo(G))
