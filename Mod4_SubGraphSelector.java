...
public class Mod4_SubGraphSelector {
    ...
    private static void GenerateFilterGraph(String filepath) throws FileNotFoundException, IOException {
       ...
       } 
       {
        ...
        // System.out.println(id);
         ...
         String substring = sCurrentLine.substring(0,sCurrentLine.indexOf(","));
          ...
          if(id.length()<LIMIT)
          {
          n1.setID(Integer.parseInt(id));g.addNode(n1);
          }
       } 
       ...
       if(sCurrentLine.substring(0,sCurrentLine.indexOf(",")).equalsIgnoreCase("1") | sCurrentLine.substring(0,sCurrentLine.indexOf(",")).equalsIgnoreCase("2"))
       {
       ...
       }
        {
           ...
           ListIterator listIterator = edgesVector.listIterator();
            ...
            //filtered graphs 
             File gf = new File("GraphFiltered"+"/"+list[i].substring(0,list[i].indexOf("."))+".xml" );
            if (!gf.exists()) {
				gf.createNewFile();
			}
            FileWriter fwf = new FileWriter(gf.getAbsoluteFile());
     ...
     private static void MergeGraphs(String filepath) throws FileNotFoundException, IOException {
         File theDir = new File("GraphsMerged");
         // if the directory  exist delete it
         if (theDir.exists()) {
             //System.out.println("creating directory: " + filepath+"/GraphsMerged");
            delete(theDir);
         }
         if (!theDir.exists()) {
             System.out.println("creating directory: " + "GraphsMerged");
             boolean result = theDir.mkdir();
             if(result) { System.out.println("DIR created"); 
             }
         }
         ...
         File file = new File("GRAPHSFILTERED");
         
         String list[];
         
        Grph g1=null;Grph g2=null;
         RegularFile f =null;
         if(file.isDirectory())
         {list=file.list();
         System.out.println("Reading graph from file");
          f = new RegularFile("GRAPHSFILTERED/"+list[0]);
             try {
                 g1 =  new GrphBinaryReader().readGraph(f);
             } catch (ParseException ex) {
                 Logger.getLogger(Mod3_GraphMerger.class.getName()).log(Level.SEVERE, null, ex);
	  ...
	  for(int i=1;i<list.length;i++){
              f = new RegularFile("GRAPHSFILTERED/"+list[i]);
             try {
                 g2=new GrphBinaryReader().readGraph(f);
             } catch (ParseException ex) {
                 Logger.getLogger(Mod3_GraphMerger.class.getName()).log(Level.SEVERE, null, ex);
             } catch (GraphBuildException ex) {
                 Logger.getLogger(Mod3_GraphMerger.class.getName()).log(Level.SEVERE, null, ex);
             ...
             IntSet vertices1 = g1.getVertices();
             IntSet vertices2 = g2.getVertices();
         ...
         g1.display();
       RegularFile   f1 = new RegularFile("GraphsMerged"+"/"+"MERGEDGRAPH.dhdf");
            new GrphBinaryWriter().writeGraph(g1, f1);
            
            
             File gf = new File("GraphsMerged"+"/"+"MERGEDGRAPH.xml" );
            if (!gf.exists()) {
				gf.createNewFile();
			}
            FileWriter fwf = new FileWriter(gf.getAbsoluteFile());
            BufferedWriter bbwf = new BufferedWriter(fwf);
            bbwf.write(g1.toGraphML());
            bbwf.close();
            
            
            gf = new File("GraphsMerged"+"/"+"MERGEDGRAPH.txt" );
            if (!gf.exists()) {
				gf.createNewFile();
			}
             fwf = new FileWriter(gf.getAbsoluteFile());
              bbwf = new BufferedWriter(fwf);
            bbwf.write(g1.toGrphText());
            bbwf.close();
            
     }
          
            
 private static void SelectSubgraph(String filepath) throws FileNotFoundException, IOException, ParseException, GraphBuildException 
 {
         File theDir = new File("SUBGRAPH_SELECTED");
         // if the directory  exist delete it
         if (theDir.exists()) {
              delete(theDir);
         }
         if (!theDir.exists()) {
             System.out.println("creating directory: " +"SUBGRAPH_SELECTED");
             boolean result = theDir.mkdir();
             if(result) { System.out.println("DIR created"); 
             }
         }
	...
	new DFSAlgorithm().compute(g, 0, new GraphSearchListener() {
		...
		System.out.println("search terminated vvvvvvv");
         ...
         //IntSet connectedComponentContaining = g.getConnectedComponentContaining(0, AbstractDegreeAlgorithm.DIRECTION.out);
         IntSet connectedComponentContaining = g.getConnectedComponentContaining(1, AbstractDegreeAlgorithm.DIRECTION.out);
         
         //System.out.println(connectedComponentContaining.toString());
         System.out.println(connectedComponentContaining.toString());
        Grph subgraphInducedByVertices = g.getSubgraphInducedByVertices(connectedComponentContaining);
        subgraphInducedByVertices.display();
        
        
        RegularFile   f1 = new RegularFile("SUBGRAPH_SELECTED"+"/"+"SUBGRAPH_SELECTED.dhdf");
            new GrphBinaryWriter().writeGraph(subgraphInducedByVertices, f1);
        
        File gf = new File("SUBGRAPH_SELECTED"+"/"+"SUBGRAPH_SELECTED.xml" );
            if (!gf.exists()) {
				gf.createNewFile();
			}
            FileWriter fwf = new FileWriter(gf.getAbsoluteFile());
            BufferedWriter bbwf = new BufferedWriter(fwf);
            bbwf.write(subgraphInducedByVertices.toGraphML());
            bbwf.close();
            
            
             gf = new File("SUBGRAPH_SELECTED"+"/"+"SUBGRAPH_SELECTED.txt" );
            if (!gf.exists()) {
				gf.createNewFile();
			}
            fwf = new FileWriter(gf.getAbsoluteFile());
              bbwf = new BufferedWriter(fwf);
            bbwf.write(subgraphInducedByVertices.toGrphText());
            bbwf.close();
        ...
        public static void main(String args[]) throws FileNotFoundException, IOException, ParseException, GraphBuildException{
            ...
            SelectSubgraph(chooser.getSelectedFile().toString());
